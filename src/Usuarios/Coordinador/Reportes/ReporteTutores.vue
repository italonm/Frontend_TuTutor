                    ...
                    >
                      ...
                      <v-date-picker v-model="startTutor" no-title scrollable>
                        ...
                        <v-btn
                          text
                          color="primary"
                          @click="registrarFechaInicioTutor(startMenu,startTutor)"
                        >OK</v-btn>
                      </v-date-picker>
                    </v-menu>
                  </v-card>
                    ...
                    >
                      ...
                      <v-date-picker v-model="endTutor" no-title scrollable>
                        ...
                        <v-btn
                          text
                          color="primary"
                          @click="registrarFechaFinTutor(endMenu,endTutor)"
                        >OK</v-btn>
                      </v-date-picker>
                    </v-menu>
                  </v-card>
          ...
          <v-card class="pa-md-4 mx-lg-auto" :height="500" style="overflow:auto;border-radius:0">
            <div v-show="mostrarReporte">
              ...
              <div id="chartMotivosRechazo">
                <apexchart
                  v-show="!showTable"
                  ref="pieMotivoRechazo"
                  type="pie"
                  width="380"
                  :options="chartOptionsMotivosRechazo"
                  :series="seriesMotivosRechazo"
                ></apexchart>
              </div>
            </div>
            <div v-show="!mostrarReporte">
              <v-alert
                ...
                prominent
                border="left"
              >Para mostrar el Reporte es necesario que seleccione un rango de Fechas.</v-alert>
            </div>
          </v-card>
        </el-col>
      </el-row>
...
export default {
  ...
  methods: {
    ...
    ReporteGeneralFormalInformal(TutorOGeneral) {
      ...
      axios
        ...
        .then((res) => {
          ...
          this.$refs.demoChartArea.updateOptions({
            colors: ["#008FFB", "#00E396", "#FFB000", "#FF1752"],
            xaxis: { categories: this.ReporteFormalInformal["labels_tiempo"] },
          });
        })
        .catch((error) => console.log(error));
    },
    ...
    reporteMotivosDeRechazo(Tutor) {
      this.enviarDatosMotivoRechazo["id_program"] = localStorage.getItem(
        "Id_facultad"
      );
      ...
      axios
        .post(
          "coordinator/show_reason_rejection_report/",
          this.enviarDatosMotivoRechazo
        )
        .then((res) => {
