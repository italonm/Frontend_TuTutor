...
export default {
    ...
    methods:{
        listarUnidades(){
            var that = this
            axios
            .get("/coordinator/show_support_units/" + localStorage.getItem("Id_institución"))
            .then(res=>{                
        ...
        guardar(){
            ...
            if (this.valid && (this.apoyo.id!==undefined)) {
                var that = this
                this.$confirm(
                    "¿Está seguro de derivar esta asignación con " + this.name + "? " + "Se eliminarán todas las citas pendientes",
                    "Advertencia",
                    {
                ...
                .then(() => {
                    ...
                    axios
                    .post("http://184.73.231.88:7002/api/tutor/derive_assignment/", that.derivado)
                    .then(()=>{
                        this.$message({ type: "success", message: "Se ha derivado exitosamente" });
                        that.apoyo = []
                        this.$emit("reset")
                        this.$emit("resetDialog");                     
                    })
