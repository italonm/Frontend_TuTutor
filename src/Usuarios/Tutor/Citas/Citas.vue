<template>
  <el-container direction="vertical">
    ...
    <v-card max-width="1200" class="ml-4">
      <v-row>
        <v-card-title class="cardAdd layout justify-center ml-3">Próximas citas</v-card-title>
      ...
      <v-expand-transition>
        <div v-show="show">
          ...
          <v-list flat subheader height="460px" class="scroll">
            <v-list-item-group subgroup color="blue" multiple>
              <template v-if="Object.keys(grupoProximasCitas).length == 0">
                <v-list-item-content style="color: gray">
                  <v-subheader
                    style="border-bottom: 2px solid blue;color:#205BD9;font-weight:bold;"
                  >No hay citas agendadas actualmente</v-subheader>
                </v-list-item-content>
              </template>
                ...
                <template v-for="(cita, index) in prox">
                  ...
                  <v-list :key="'C'+cita.id_session">
                    <v-list-group sub-group no-action>
                      ...
                      <v-list-item v-for="(datosAlumno, i) in cita.alumnos" :key="'D'+i" link>
                        <v-list-item-title
                          v-text="datosAlumno.codigo + ' ' + datosAlumno.nombre + ' ' + datosAlumno.apellidos"
                        ></v-list-item-title>
                      </v-list-item>
                    </v-list-group>
                  </v-list>
    ...
    <v-card max-width="1200" class="ml-4">
      <v-row>
        <v-card-title class="cardAdd layout justify-center ml-3">Citas pasadas sin resultado</v-card-title>
        <v-card-actions class="cardAdd mr-3">
          <v-btn color="white" icon @click="show2 = !show2">
            ...
            <v-list-item-group subgroup color="blue" multiple>
              <template v-if="Object.keys(grupoCitasSinResultados).length == 0">
                <v-list-item-content style="color: gray">
                  <v-subheader
                    style="border-bottom: 2px solid blue;color:#205BD9;font-weight:bold;"
                  >No hay citas pendientes a mostrar</v-subheader>
                </v-list-item-content>
              </template>
                ...
                <template v-for="(cita, index) in proxSin">
                  ...
                  <v-list :key="'H'+cita.id_session">
                    <v-list-group sub-group no-action>
                      ...
                      <v-list-item v-for="(datosAlumno, i) in cita.alumnos" :key="'I'+i" link>
                        <v-list-item-title
                          v-text="datosAlumno.codigo + ' ' + datosAlumno.nombre + ' ' + datosAlumno.apellidos"
                        ></v-list-item-title>
                      </v-list-item>
...
export default {
  ...
  methods: {
    listar() {
      axios
        .get(
          "/tutor/show_list_formal_sessions/" +
    ...
    ordenarCitas() {
      ...
      this.listaCitasSinResultados.forEach(function (val) {
        ...
        var date = new Intl.DateTimeFormat("es-PE", options).format(birthday);
    ...
    editar(item) {
      axios
        ...
        .then((res) => {
          ...
          axios
            .get("/tutor/show_result/" + item.id_session)
            .then((res) => {
              ...
              if (this.resultado.comentario !== null) {
                ...
                this.dialog = true;
              } else this.action = "Crear resultado de la cita";
              this.dialog = true;
              axios
                .get(
    ...
    cancelarCita() {
      ...
      axios
        .post("/tutor/cancel_session/", this.cancelarCitaDatos)
        .then((res) => {
          console.log(res);
          this.listar();
          this.$message({
            message:
              "Cancelación exitosa. Se le envió un correo electrónico al estudiante",
            type: "success",
          });
          this.dialogCancelar = false;
          this.$emit("resetDialog", this.dialogCancelar);
