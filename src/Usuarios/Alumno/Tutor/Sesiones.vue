    ...
    <el-row>
      ...
      <el-col :span="12">
        <div class="grid-content">
          ...
          <v-col v-for="(activity, i) in plan" :key="i" cols="12">
            <v-card :color="colors[i+1]" dark>
            ...
            <v-timeline-item v-for="(session, i) in sessions" :key="i" :color="colors[i]" small>
              <template v-slot:opposite>
                <span
                  ...
                  v-text="session.fecha"
                ></span>
              </template>
              <v-card :color="colors[i]" dark>
                <v-card-title class="title">{{session.lugar}}</v-card-title>
                <v-card-text class="white text--primary">
                  <div v-if="session.asistencia == 1">
                    <i class="fas fa-check">&nbsp;Asistió</i>
                    <div>Resultado: {{session.resultado}}</div>
                  </div>
                  <div v-else>
                    <i class="fas fa-times">&nbsp;No asistió</i>
                    <div>Resultado pendiente</div>
                  </div>
                </v-card-text>
              </v-card>
            </v-timeline-item>
...
export default {
  ...
  methods: {
    listar() {
      // console.log(localStorage.getItem("Id_tutoria").toString());
      // console.log(localStorage.getItem("Id_usuario"));
      // var token = "6Q7Q6NRPQFjNJFTyM2zGmVUtrFJ4kB9tK4Rc1aEvZPcV";
      ...
      axios
        ...
        .then(res => {
          this.sessions = res.data.data1[0];
          this.plan = res.data.data1[1];
