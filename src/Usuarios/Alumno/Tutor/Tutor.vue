      ...
      <v-layout wrap justify-space-around>
        <v-flex v-for="tutoria in miFiltrado(tutorias)" :key="tutoria.t_email">
          <v-card class="mx-auto" max-width="300">
            ...
            >
              <v-card-title>Tutoría grupal</v-card-title>
            </v-img>
            ...
            <v-card-text class="text--primary">
              ...
              <div>Una pequeña descripción de la tutoría</div>
            </v-card-text>
            <v-card-actions>
              <v-btn color="green" text @click="editar(tutoria)">
                <i class="fas fa-calendar-times"></i>Agendar cita
              </v-btn>
              <v-btn color="#212DA8" text @click="editar(tutoria)"
                >Sesiones</v-btn
              >
            </v-card-actions>
          </v-card>
...
export default {
  ...
  methods: {
    ...
    editar(tutoria) {
      ...
      console.log("hehe");
      this.form = Object.assign({}, tutoria);
      this.dialog = true;
    },
